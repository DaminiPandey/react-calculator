...
const handleKeydown = (e) => { 
  ...
  if (!isNumber && !isOperator && !isClear && !isEnter) 
    return;
...
const handleNumberClick = (value) => {
  if(value !== '='){
    if(!operator){
      if(leftOperand == '0' || leftOperand == 'Infinity' || leftOperand == 'NaN'){
      ...
      if(rightOperand == '0' || leftOperand == 'Infinity' || leftOperand == 'NaN'){
        ...
        console.log(leftOperand.includes('.'), value === '.');
...
const handleOperatorClick = (value, key) => {
  if(leftOperand && rightOperand && operator && value !== '=' && key !== 'Enter'){
...
const handleClick = (value, key) => {
  ...
  return (
     <div className='calculator p-4 bg-[#E8F1F2] text-center m-auto mt-8' tabIndex='0' ref={calRef} onKeyDown={handleKeydown}>
          ...
          <div className='grid grid-cols-4'>
            <Buttons result={result} setResult={setResult} handleClick={handleClick} />
          </div>
          <div className='pt-4'> 
            <p className='font-bold text-[#008090]'><span className='text-[#008090]'>Built by </span><a href='https://github.com/DaminiPandey' className='text-[#104B5F] cursor-pointer hover:underline'>Damini Pandey </a> | <a className='text-[#104B5F] cursor-pointer hover:underline' href='https://github.com/DaminiPandey/react-calculator'>Source Code</a></p>
          </div>
